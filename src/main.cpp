...
const std::array<MaterialPreset, 4> MATERIAL_PRESETS = {{
    {"Plastic", glm::vec3(1.0f, 0.42f, 0.22f), glm::vec3(0.0f, 0.72f, 0.68f), glm::vec3(0.55f, 0.55f, 0.58f), 32.0f, 0.50f, 0.12f, 0.0f, 0.45f, 1.0f, 0.45f},
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
    ...
    if (fbMouseX >= sceneWidth) {
        uiScrollOffset = std::clamp(uiScrollOffset - static_cast<float>(yoffset) * 42.0f, 0.0f, 260.0f);
    }
}
...
void drawScene(const Shader& shader, bool depthOnly) {
    ...
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(-halfRoomWidth - wallThickness * 0.5f, roomCenterY, 0.0f));
    model = glm::scale(model, glm::vec3(wallThickness, ROOM_HEIGHT, ROOM_DEPTH));
    shader.setMat4("model", model);
    if (!depthOnly) {
...
void drawControlPanel(UI& ui, GLFWwindow* window) {
    ...
    auto section = [&](const std::string& title) {
        ...
        cursorY += 14.0f;
        ui.text(contentX, cursorY, title, glm::vec4(0.88f, 0.94f, 0.95f, 1.0f), 1.65f);
        cursorY += 25.0f;
    };

    ui.panel();
    ui.text(contentX, cursorY, "BANG DIEU KHIEN", glm::vec4(0.0f, 0.85f, 0.85f, 1.0f), 2.55f);
    cursorY += 34.0f;
    ui.text(contentX, cursorY, "PHONG LIGHTING LAB", glm::vec4(0.65f, 0.72f, 0.75f, 1.0f), 1.55f);
    ui.text(panelX + panelW - 110.0f, panelY + 14.0f, "SCROLL", glm::vec4(0.45f, 0.55f, 0.57f, 1.0f), 1.15f);
    cursorY += 22.0f;

    section("SHADING MODEL");
    const float gap = 10.0f;
    const float buttonW = (contentW - gap * 2.0f) / 3.0f;
    if (ui.button(contentX, cursorY, buttonW, 42.0f, "PHONG", renderMode == 0)) renderMode = 0;
    if (ui.button(contentX + buttonW + gap, cursorY, buttonW, 42.0f, "BLINN", renderMode == 1)) renderMode = 1;
    if (ui.button(contentX + (buttonW + gap) * 2.0f, cursorY, buttonW, 42.0f, "PBR", renderMode == 2)) renderMode = 2;
    cursorY += 44.0f;

    section("LIGHT COMPONENTS");
    if (ui.checkbox(contentX, cursorY, "AMBIENT", enableAmbient)) enableAmbient = !enableAmbient;
    cursorY += 27.0f;
    if (ui.checkbox(contentX, cursorY, "DIFFUSE", enableDiffuse)) enableDiffuse = !enableDiffuse;
    cursorY += 27.0f;
    if (ui.checkbox(contentX, cursorY, "SPECULAR", enableSpecular)) enableSpecular = !enableSpecular;
    cursorY += 17.0f;

    section("SHADOW MAPPING");
    if (ui.checkbox(contentX, cursorY, "SHADOW", enableShadow)) enableShadow = !enableShadow;
    cursorY += 29.0f;
    ui.slider(contentX, cursorY, contentW, "SHADOW STRENGTH", shadowStrength, 0.0f, 1.0f);
    cursorY += 39.0f;

    section("GLOBAL ILLUMINATION");
    if (ui.checkbox(contentX, cursorY, "GI APPROX", enableGI)) enableGI = !enableGI;
    cursorY += 29.0f;
    ...
    ui.text(contentX + 200.0f, cursorY + 10.0f, MATERIAL_PRESETS[currentMaterialIndex].name, glm::vec4(0.65f, 0.72f, 0.75f, 1.0f), 1.35f);
    cursorY += 33.0f;

    section("AI-GI LITE");
    if (ui.checkbox(contentX, cursorY, "AI-GI LITE", enableAIGI)) enableAIGI = !enableAIGI;
    cursorY += 29.0f;
    ui.slider(contentX, cursorY, contentW, "AI STRENGTH", aiStrength, 0.0f, 1.0f);
    cursorY += 39.0f;

    section("LIGHT / CAMERA");
    if (ui.checkbox(contentX, cursorY, "ORBIT LIGHT", autoOrbitLight)) autoOrbitLight = !autoOrbitLight;
    cursorY += 27.0f;
    if (ui.checkbox(contentX, cursorY, "ATTENUATION", enableAttenuation)) enableAttenuation = !enableAttenuation;
    cursorY += 29.0f;
    ui.slider(contentX, cursorY, contentW, "LIGHT INTENSITY", lightIntensity, 0.5f, 10.0f);
    cursorY += 39.0f;
    if (ui.button(contentX, cursorY, 122.0f, 38.0f, "RESET", false)) resetScene();
}
